...
///
/// This struct must to be passed as a (unused) reference to any function that requires round toward zero
/// for correct operation. The struct serves as a proof that RTZ is set and we rely on the lifetime introduced
/// by the reference to enforce the ordering of the FPCR operations relative to the multiplication. This
/// way we can prevent the reset of FPCR to bubble up in front of the multiplication.
///
/// This type provides RAII-style management of the AArch64 FPCR (Floating-point Control Register),
/// specifically for controlling the rounding mode. When created, it sets the rounding mode to
/// "round toward zero" and restores the previous mode when dropped.
///
/// # Safety
///
/// This type is not Send because FPCR is a per-core / per OS-thread register. The PhantomData<*mut ()> ensures this.
/// Only one instance can exist per thread at a time, enforced by the FPCR_OWNED thread-local.
///
/// # Design Notes
///
/// This type
...
impl RTZ {
    /// Attempts to create a new RTZ instance, setting the FPCR to round-toward-zero mode.
    ///
    /// Returns None if another RTZ instance already exists in this thread.
    ///
    /// # Safety
    ///
    /// This function uses inline assembly to modify the FPCR register. The operations are safe
    /// when used as intended through this API, as it maintains the following invariants:
    ...
    ///
    /// This method is primarily intended for debugging and verification purposes.
    #[cfg(target_arch = "aarch64")]
    #[inline]
    ...
    /// It should be used with caution as improper values can affect floating-point behavior.
    ///
    /// # Safety
    ///
