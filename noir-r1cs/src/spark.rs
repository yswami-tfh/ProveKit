...
pub fn prove_spark(
    ...
    row_randomness: &[FieldElement],
    col_randomness: &[FieldElement],
...
pub fn run_sumcheck_prover_spark(
    ...
    loop {
        ...
        saved_val_for_sumcheck_equality_assertion =
            eval_cubic_poly(&hhat_i_coeffs, &alpha_i_wrapped_in_vector[0]);
...
pub fn verify_spark_sumcheck(
    ...
    let (randomness, _last_sumcheck_value) =
    ...
    let (_folding_randomness, _deferred) = verifier
        .verify(
            arthur,
...
pub fn run_sumcheck_verifier_spark(
    ...
    for (i, _) in (0..variable_count).enumerate() {
