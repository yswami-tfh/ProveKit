...
/// Add the witnesses and constraints for a [BinOp] (i.e. AND, XOR). Uses a digital decomposition of
/// the operands and output into [NUM_DIGITS] digits of [BINOP_ATOMIC_BITS] bits each, followed by a
/// lookup table of size 2x [BINOP_ATOMIC_BITS].
pub(crate) fn add_binop(
    ...
    // Collect all witnesses that require digital decomposition (constants are decomposed
    // separately).
    ...
    // Match up the digit witnesses and the digits of decompositions of constants to obtain a
    // decomposed version of the inputs and outputs.
    let mut inputs_and_outputs_atomic = vec![];
    // Track how many witness digital decompositions we've seen so far (for associating the digit
    // witnesses with the original witnesses).
    ...
    for (lh, rh, _output) in inputs_and_outputs {
        ...
        lh_atoms.into_iter().zip(rh_atoms.into_iter()).zip(output_atoms.into_iter()).for_each(|((lh, rh), output)| {
...
// Add and return a new witness `denominator` and constrain it to represent (assuming `output` is a
// witness):
// `w[sz_challenge] - (w[lh_operand] + w[rs_challenge] * w[rh_operand] + w[rs_challenge_sqrd] * w[output])`
// where `w` is the witness vector. If `output` is a constant, then the `rs_challenge_sqrd` is
// instead scaled by that constant. Finally, adds a new witness for the inverse of `denominator`,
