...
class FRIRound:
    def __init__(self, 
                 input_polynomial : PolynomialRing,
                 generator : int,       # must have order 2^k
...
class FRIOPP:
    ...
    def __init__(self, 
        ...
        self.rounds = list()
        first_round = FRIRound(input_polynomial, generator, rate_factor, folding_factor)
    ...
    def verify(self, max_degree, query_count = 10):
...
if __name__ == '__main__':
    def run_test_friopp(code_dimension=2048, rate_factor=4, folding_factor=4, ood_count = 5):
        ...
        omega = omega**(cofactor*2**(twodicity - log(code_dimension, 2)))
        message_size =  code_dimension//rate_factor - int(random()*(code_dimension//10))
        input = PolyRing.random_element(message_size)

        iopp = FRIOPP(input_polynomial=input, generator=omega, rate_factor=rate_factor, folding_factor=folding_factor)

        assert iopp.verify(message_size), "FRI IOPP varification should work"
        assert not iopp.verify(message_size // folding_factor), "FRI IOPP varification should fail if the max degree is 1/folding factor away"
    ...
    def run_test_fri_round(code_dimension=2048, rate_factor=4, folding_factor=4, ood_count = 5):
        ...
        message_size =  code_dimension//rate_factor

        input = PolyRing.random_element(message_size-1)

        first_oracle = FRIRound(input, omega, rate_factor, 2)
        ...
        FRIRound.consistancy_cross_check(second_oracle, first_oracle, folding_challenge=folding_randomness, queries=5, ood_response=ood_response)
